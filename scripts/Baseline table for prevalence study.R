...
##Plot##
p = ggplot(data.frame(Output), aes(x=as.factor(prevalence), y=TLPmedian)) +
  geom_boxplot()

p +labs(title="Plot of mean Ct by dilution level for each Ct category",
        x ="Ct Category", y = "Cycle threshold", fill = "Dilution level")
